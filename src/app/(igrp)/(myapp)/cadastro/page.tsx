...
import {
  useClientes, useCriarCliente, useAtualizarCliente,
  useFornecedores, useCriarFornecedor, useAtualizarFornecedor,
} from "@/hooks/use-cadastro";
import type { Cliente, Fornecedor, TipoEntidade } from "@/app/(myapp)/types/efatura";

// ── Cape Verde geographic data ────────────────────────────────

const ILHAS = ["SANTIAGO", "SÃO VICENTE", "SÃO ANTÃO", "FOGO", "BRAVA", "MAIO", "SAL", "BOA VISTA", "SÃO NICOLAU"];
...
function emptyCliente(): Omit<Cliente, "id" | "createdAt" | "updatedAt" | "createdBy" | "lastModifiedBy"> {
  return {
    nome: "", tipoEntidade: "COLETIVO", ativo: true,
    codigo: "", nif: "", email: "", telefone: "",
    pessoaContacto: "", morada: "", endereco: "",
    ilha: "SANTIAGO", conselho: "PRAIA", freguesia: "", localidade: "",
    descricao: "", aplicarImpostos: false, enquadramento: "",
  };
}

function emptyFornecedor(): Omit<Fornecedor, "id" | "createdAt" | "updatedAt" | "createdBy" | "lastModifiedBy"> {
  return {
    nome: "", tipoEntidade: "COLETIVO", ativo: true,
    codigo: "", nif: "", email: "", telefone: "",
    morada: "", ilha: "SANTIAGO", conselho: "PRAIA",
...
function ClienteModal({
  ...
  const [form, setForm] = useState<Omit<Cliente, "id" | "createdAt" | "updatedAt" | "createdBy" | "lastModifiedBy">>(
    cliente ? {
      nome: cliente.nome, tipoEntidade: cliente.tipoEntidade, ativo: cliente.ativo,
      codigo: cliente.codigo ?? "", nif: cliente.nif ?? "", email: cliente.email ?? "",
      telefone: cliente.telefone ?? "", pessoaContacto: cliente.pessoaContacto ?? "",
      morada: cliente.morada ?? "", endereco: cliente.endereco ?? "",
      ilha: cliente.ilha ?? "SANTIAGO", conselho: cliente.conselho ?? "PRAIA",
      freguesia: cliente.freguesia ?? "", localidade: cliente.localidade ?? "",
      descricao: cliente.descricao ?? "", aplicarImpostos: cliente.aplicarImpostos ?? false,
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-5">
            ...
            <div className="flex-1 grid grid-cols-3 gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">Código <span className="text-red-400">*</span></label>
                <input
                  disabled value={form.codigo ?? ""}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">Tipo Cliente <span className="text-red-400">*</span></label>
                <select
                  value={form.tipoEntidade}
                  onChange={(e) => setForm((p) => ({ ...p, tipoEntidade: e.target.value as TipoEntidade }))}
                  className="h-8 rounded border border-gray-300 bg-white px-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                >
                  {TIPOS_ENTIDADE.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
              </div>
              <div className="flex flex-col gap-1 col-span-2">
                <label className="text-xs text-gray-500">Nome <span className="text-red-400">*</span></label>
                <input
                  value={form.nome}
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold text-gray-600">Localidade/Morada</span>
            </div>
            <div className="grid grid-cols-4 gap-3">
              <div className="flex flex-col gap-1">
                ...
                <select
                  ...
                  onChange={(e) => setForm((p) => ({ ...p, ilha: e.target.value, conselho: "" }))}
                  className="h-8 rounded border border-gray-300 bg-white px-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                >
              ...
              <div className="flex flex-col gap-1">
                ...
                <select
                  value={form.conselho ?? ""}
                  onChange={(e) => setForm((p) => ({ ...p, conselho: e.target.value }))}
                  className="h-8 rounded border border-gray-300 bg-white px-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                >
                  <option value=""></option>
          ...
          {/* Endereço */}
          <div className="flex flex-col gap-1">
            <label className="text-xs text-gray-500">Endereço <span className="text-red-400">*</span></label>
            <input
              value={form.endereco ?? ""}
...
function FornecedorModal({
  ...
  const [form, setForm] = useState<Omit<Fornecedor, "id" | "createdAt" | "updatedAt" | "createdBy" | "lastModifiedBy">>(
    fornecedor ? {
      nome: fornecedor.nome, tipoEntidade: fornecedor.tipoEntidade, ativo: fornecedor.ativo,
      codigo: fornecedor.codigo ?? "", nif: fornecedor.nif ?? "", email: fornecedor.email ?? "",
      telefone: fornecedor.telefone ?? "", morada: fornecedor.morada ?? "",
      ilha: fornecedor.ilha ?? "SANTIAGO", conselho: fornecedor.conselho ?? "PRAIA",
      freguesia: fornecedor.freguesia ?? "", localidade: fornecedor.localidade ?? "",
    } : emptyFornecedor()
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-2xl rounded-lg border border-gray-200 bg-white shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          <div className="flex gap-5">
            ...
            <div className="flex-1 grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">Código <span className="text-red-400">*</span></label>
                <input disabled value={form.codigo ?? ""} className="h-8 rounded border border-gray-200 bg-gray-50 px-2.5 text-xs text-gray-500" />
              </div>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">NIF <span className="text-red-400">*</span></label>
                <input
                  value={form.nif ?? ""}
                  onChange={(e) => setForm((p) => ({ ...p, nif: e.target.value }))}
                  className="h-8 rounded border border-gray-300 bg-white px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
        ...
        <div className="flex items-center justify-between border-t border-gray-200 px-6 py-4">
          <button onClick={onClose} className="rounded border border-gray-300 px-5 py-1.5 text-xs text-gray-600 hover:bg-gray-50">Fechar</button>
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded bg-indigo-600 px-6 py-1.5 text-xs font-semibold text-white hover:bg-indigo-700 disabled:opacity-60"
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200">
        {isLoading ? (
          ...
          <div className="py-8 text-center text-sm text-gray-400">Nenhum cliente encontrado</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="w-10 px-3 py-2.5 text-center font-medium text-gray-500">#</th>
                <th className="px-3 py-2.5 text-left font-medium text-gray-600">Código</th>
                <th className="px-3 py-2.5 text-left font-medium text-gray-600">Nome</th>
                <th className="px-3 py-2.5 text-left font-medium text-gray-600">NIF</th>
                <th className="px-3 py-2.5 text-left font-medium text-gray-600">Email</th>
                <th className="px-3 py-2.5 text-left font-medium text-gray-600">Telefone</th>
                <th className="px-3 py-2.5 text-center font-medium text-gray-600">Tipo</th>
                <th className="px-3 py-2.5 text-center font-medium text-gray-600">Estado</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {clientes.map((c, idx) => (
                <tr key={c.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2.5 font-mono text-xs text-gray-700">{c.codigo ?? "—"}</td>
                  <td className="px-3 py-2.5 font-medium text-gray-800">{c.nome}</td>
                  <td className="px-3 py-2.5 text-xs text-gray-600">{c.nif ?? "—"}</td>
                  <td className="px-3 py-2.5 text-xs text-gray-600">{c.email ?? "—"}</td>
                  <td className="px-3 py-2.5 text-xs text-gray-600">{c.telefone ?? "—"}</td>
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200">
        {isLoading ? (
          ...
          <div className="py-8 text-center text-sm text-gray-400">Nenhum fornecedor encontrado</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
            ...
            <tbody className="divide-y divide-gray-100">
              {fornecedores.map((f, idx) => (
                <tr key={f.id} className="hover:bg-gray-50">
                  <td className="px-3 py-2.5 text-center text-xs text-gray-400">{idx + 1}</td>
                  <td className="px-3 py-2.5 font-mono text-xs text-gray-700">{f.codigo ?? "—"}</td>
                  <td className="px-3 py-2.5 font-medium text-gray-800">{f.nome}</td>
                  <td className="px-3 py-2.5 text-xs text-gray-600">{f.nif ?? "—"}</td>
                  <td className="px-3 py-2.5 text-xs text-gray-600">{f.email ?? "—"}</td>
                  <td className="px-3 py-2.5 text-xs text-gray-600">{f.telefone ?? "—"}</td>
                  <td className="px-3 py-2.5 text-center">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium ${
    ...
    <div className="flex flex-col gap-0 p-0">
      <div className="border-b border-gray-200 bg-white px-6 py-2.5">
        <nav className="flex items-center gap-1 text-xs text-gray-500">
          <a href="/" className="hover:text-gray-700">Página Inicial</a>
          <span>/</span>
