...
import { useFaturaVenda, useConfirmarFaturaVenda } from "@/hooks/use-faturas-venda";
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/30 pt-32">
      <div className="w-full max-w-lg rounded-lg border border-gray-200 bg-white shadow-xl">
        <div className="flex items-center border-b border-gray-200 px-4 py-3">
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-4 w-4 text-gray-400 mr-2">
...
function LinhaProduto({
  ...
  const total = (item.quantidade || 0) * (item.precoUnitario || 0) * (1 - (item.descontoComercialPerc || 0) / 100);
    ...
    <tr className="border-b border-gray-100 hover:bg-gray-50/50">
      ...
      <td className="px-2 py-1.5">
        <input
          ...
          max={100}
          step="0.01"
          onChange={(e) => onChange("descontoComercialPerc", parseFloat(e.target.value))}
          className="w-14 rounded border-0 bg-transparent text-right text-xs focus:outline-none focus:bg-gray-50 focus:ring-1 focus:ring-blue-300 px-1.5 py-0.5"
        />
      </td>
      <td className="px-2 py-1.5">
...
export default function FaturaVendaDetailPage() {
  ...
  const { mutateAsync: confirmar, isPending: isConfirming } = useConfirmarFaturaVenda();
  ...
  function updateItem(index: number, field: keyof ItemFaturaVenda, value: string | number) {
  ...
  const totalIva = itens.reduce((acc, item) => {
    const base = (item.quantidade || 0) * (item.precoUnitario || 0) * (1 - (item.descontoComercialPerc || 0) / 100);
  ...
  async function handleSave() {
    ...
    try {
      await faturasVendaApi.atualizar(id, {
        ...
        observacoes: nota,
        itens: itens.map(({ desig, descricao, quantidade, precoUnitario, descontoComercialPerc, percentagemIva, unidade, produtoId, codigoArtigo }) => ({
  ...
  if (isError || !fatura) {
    return (
      <div className="flex flex-col items-center gap-3 py-32">
        ...
        <button onClick={() => router.back()} className="text-xs text-blue-500 hover:underline">
          Voltar
        </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {showProdutos && (
        <ProdutoSearch onSelect={addProduto} onClose={() => setShowProdutos(false)} />
      )}
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="relative flex h-8 items-center rounded border border-gray-300 bg-white px-2">
                  ...
                  />
                  <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-3.5 w-3.5 text-gray-400 absolute right-2">
            ...
            <div className="flex items-end gap-3">
              <div className="relative flex-1">
                ...
                <div className="flex h-8 items-center rounded border border-gray-300 bg-white">
                  <input
                    value={clienteSelecionado ? `${clienteSelecionado.nome} - ${clienteSelecionado.nif ?? ""} -` : clienteSearch}
                    onChange={(e) => {
                  ...
                  {selectedClienteId && (
                    <button
                      onClick={() => { setSelectedClienteId(undefined); setClienteSearch(""); }}
                ...
                {showClienteDD && clientesFiltrados.length > 0 && (
                  <>
                    <div className="fixed inset-0 z-10" onClick={() => setShowClienteDD(false)} />
              ...
              <table className="w-full min-w-[700px] text-xs">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-2 py-2 text-left text-gray-500 font-medium">Desig.</th>
                    <th className="w-14 px-2 py-2 text-center text-gray-500 font-medium">Qtd.</th>
                    <th className="w-14 px-2 py-2 text-center text-gray-500 font-medium">Unid</th>
                    <th className="w-20 px-2 py-2 text-right text-gray-500 font-medium">Preço/Unid</th>
                    <th className="w-20 px-2 py-2 text-right text-gray-500 font-medium">% Desc. Comercial</th>
                    <th className="px-2 py-2 text-left text-gray-500 font-medium">Descrição</th>
                    <th className="w-20 px-2 py-2 text-center text-gray-500 font-medium">Imposto</th>
                    <th className="w-20 px-2 py-2 text-right text-gray-500 font-medium">Total</th>
                    <th className="w-8 px-2 py-2 text-gray-500 font-medium">Ação</th>
                  </tr>
                </thead>
                <tbody>
                  {itens.length === 0 ? (
                  ...
                  <tr className="border-t border-gray-200 bg-blue-500 text-white text-xs font-semibold">
                    ...
                    <td colSpan={7} className="px-2 py-2">SubTotal:</td>
                    <td className="px-2 py-2 text-right">{fmt(totalDesconto)}</td>
                    <td className="px-2 py-2 text-right">{fmt(subtotal - totalDesconto)}</td>
                  </tr>

                  {/* Total row */}
                  <tr className="border-t border-gray-200 bg-blue-600 text-white text-xs font-semibold">
                    <td className="px-2 py-2 text-center">#</td>
                    <td colSpan={8} className="px-2 py-2">Total a pagar:</td>
