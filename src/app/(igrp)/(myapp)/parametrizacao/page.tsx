...
import type { SerieDocumento, TaxaIva, TipoDocumento } from "@/app/(myapp)/types/efatura";
...
function TaxasIvaTab() {
  ...
  const [form, setForm] = useState<Partial<TaxaIva>>({ ativo: true, percentagem: 15 });
    ...
    <div className="space-y-4">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50/40 p-5">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <div className="flex flex-col gap-1 col-span-2">
              <label className="text-xs text-gray-500">Descrição <span className="text-red-400">*</span></label>
              <input
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-500">Percentagem (%) <span className="text-red-400">*</span></label>
              <input
                type="number" min={0} max={100} step="0.01"
                value={form.percentagem ?? ""}
                onChange={(e) => setForm((p) => ({ ...p, percentagem: parseFloat(e.target.value) }))}
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200">
        {isLoading ? (
          ...
          <div className="py-8 text-center text-sm text-gray-400">Nenhuma taxa configurada</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="px-4 py-2.5 text-left font-medium text-gray-600">Código</th>
                <th className="px-4 py-2.5 text-left font-medium text-gray-600">Descrição</th>
                <th className="px-4 py-2.5 text-right font-medium text-gray-600">Percentagem</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {taxas.map((taxa) => (
                <tr key={taxa.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2.5 font-mono text-xs text-gray-700">{taxa.codigo}</td>
                  <td className="px-4 py-2.5 text-gray-700">{taxa.descricao}</td>
                  <td className="px-4 py-2.5 text-right font-medium text-gray-800">{taxa.percentagem}%</td>
                  <td className="px-4 py-2.5 text-center">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium ${
                      taxa.ativo ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"
                    }`}>
...
function SeriesTab() {
  ...
  const save = useMutation({
    mutationFn: () => parametrizacaoApi.series.criar(form as Omit<SerieDocumento, "id">),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["series"] });
      setShowForm(false);
      setForm({ ativo: true, anoFiscal: new Date().getFullYear(), ultimoNumero: 0, tipoDocumento: "FATURA" });
    },
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-700">Séries de Documento</h2>
        <button
      ...
      {showForm && (
        <div className="rounded-lg border border-blue-200 bg-blue-50/40 p-5">
          <h3 className="mb-4 text-sm font-semibold text-gray-700">Nova Série</h3>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-500">Tipo de Documento <span className="text-red-400">*</span></label>
              <select
                value={form.tipoDocumento}
                onChange={(e) => setForm((p) => ({ ...p, tipoDocumento: e.target.value as TipoDocumento }))}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-500">Prefixo <span className="text-red-400">*</span></label>
              <input
                value={form.prefixo ?? ""}
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-gray-500">Ano Fiscal <span className="text-red-400">*</span></label>
              <input
                type="number" min={2020} max={2099}
                value={form.anoFiscal ?? ""}
                onChange={(e) => setForm((p) => ({ ...p, anoFiscal: parseInt(e.target.value) }))}
                className="h-8 rounded border border-gray-300 bg-white px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
              />
            </div>
          </div>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200">
        {isLoading ? (
          ...
          <div className="py-8 text-center text-sm text-gray-400">Nenhuma série configurada</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="px-4 py-2.5 text-left font-medium text-gray-600">Código</th>
                <th className="px-4 py-2.5 text-left font-medium text-gray-600">Descrição</th>
                <th className="px-4 py-2.5 text-left font-medium text-gray-600">Tipo Documento</th>
                <th className="px-4 py-2.5 text-center font-medium text-gray-600">Prefixo</th>
                <th className="px-4 py-2.5 text-center font-medium text-gray-600">Ano</th>
                <th className="px-4 py-2.5 text-center font-medium text-gray-600">Último Nº</th>
                <th className="px-4 py-2.5 text-center font-medium text-gray-600">Estado</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {series.map((serie) => (
                <tr key={serie.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2.5 font-mono text-xs text-gray-700">{serie.codigo}</td>
                  <td className="px-4 py-2.5 text-gray-700">{serie.descricao}</td>
                  <td className="px-4 py-2.5 text-xs text-gray-600">{serie.tipoDocumento}</td>
                  <td className="px-4 py-2.5 text-center font-mono text-xs text-gray-700">{serie.prefixo}</td>
                  <td className="px-4 py-2.5 text-center text-gray-700">{serie.anoFiscal}</td>
                  <td className="px-4 py-2.5 text-center font-medium text-gray-800">{serie.ultimoNumero}</td>
                  <td className="px-4 py-2.5 text-center">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium ${
                      serie.ativo ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"
                    }`}>
        ...
        <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
          <div className="border-b border-gray-200 px-5 py-3.5">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-sm font-semibold text-gray-800">Parametrização</h1>
            </div>
          </div>
