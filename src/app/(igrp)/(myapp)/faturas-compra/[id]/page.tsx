...
import { useFaturaCompra, useConfirmarFaturaCompra, useAtualizarFaturaCompra } from "@/hooks/use-faturas-compra";
import { useFornecedores } from "@/hooks/use-cadastro";
import type { ItemFaturaCompra } from "@/app/(myapp)/types/efatura";

function fmt(v?: number) {
  if (v === undefined || v === null) return "0";
  return new Intl.NumberFormat("pt-CV", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(v);
}
...
function LinhaItem({
  ...
  const total = (item.quantidade || 0) * (item.precoUnitario || 0) * (1 + (item.percentagemIva || 0) / 100);
  ...
  return (
    <tr className="border-b border-gray-100 hover:bg-gray-50/50">
      ...
      <td className="px-2 py-1.5">
        <input
          type="number" value={item.quantidade} min={0} step="1"
      ...
      <td className="px-2 py-1.5">
        <input
          type="number" value={item.precoUnitario} min={0} step="0.01"
      ...
      <td className="px-2 py-1.5">
        <input
          type="number" value={item.percentagemIva ?? 15} min={0} max={100} step="0.1"
          onChange={(e) => onChange("percentagemIva", parseFloat(e.target.value))}
          className="w-14 rounded border-0 bg-transparent text-right text-xs focus:outline-none focus:bg-gray-50 focus:ring-1 focus:ring-blue-300 px-1.5 py-0.5"
        />
      </td>
      <td className="px-2 py-1.5 text-right text-xs font-medium text-gray-800">{fmt(total)}</td>
      <td className="px-2 py-1.5 text-center">
        <button onClick={onRemove} className="text-gray-300 hover:text-red-500 transition-colors text-base leading-none">
          ×
        </button>
      </td>
    </tr>
...
export default function FaturaCompraDetailPage() {
  ...
  function updateItem(index: number, field: keyof ItemFaturaCompra, value: string | number) {
    ...
    try {
      await atualizar({
        ...
        data: {
          ...
          observacoes: nota,
          itens: itens.map(({ descricao, quantidade, precoUnitario, percentagemIva }) => ({
            descricao: descricao ?? "",
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center py-32 text-sm text-gray-400">A carregar fatura…</div>;
  }

  if (isError || !fatura) {
    return (
      <div className="flex flex-col items-center gap-3 py-32">
        <p className="text-red-500 text-sm">{(error as Error)?.message ?? "Fatura não encontrada"}</p>
        <button onClick={() => router.back()} className="text-xs text-blue-500 hover:underline">Voltar</button>
      </div>
    );
  }
        ...
        <div className="p-6 space-y-6">
          ...
          <section>
            <div className="flex items-center gap-2 mb-3">
              <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-4 w-4 text-gray-400">
                <path d="M3 4h14M3 8h14M3 12h14M3 16h8" />
              </svg>
              <h2 className="text-xs font-semibold uppercase tracking-wide text-gray-500">Dados de Compra</h2>
            </div>
            <div className="grid grid-cols-4 gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">Nº Documento</label>
                <input
                  disabled value={fatura.numero ?? `FC${id}`}
                  className="h-8 rounded border border-gray-200 bg-gray-50 px-2.5 text-xs text-gray-500"
                />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">Série <span className="text-red-400">*</span></label>
                <select
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">Data <span className="text-red-400">*</span></label>
                <input
                  type="date" value={data_} onChange={(e) => setData_(e.target.value)}
                  className="h-8 rounded border border-gray-300 bg-white px-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">Condições Pagamento <span className="text-red-400">*</span></label>
                <select
                  value={condicoes} onChange={(e) => setCondicoes(e.target.value)}
                  className="h-8 rounded border border-gray-300 bg-white px-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
                >
          ...
          <section>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-semibold uppercase tracking-wide text-gray-500">Dados do Fornecedor</h2>
            </div>
            <div className="relative">
              <label className="mb-1 block text-xs text-gray-500">Fornecedor</label>
              <div className="flex h-8 items-center rounded border border-gray-300 bg-white">
                <input
                  value={fornecedorSelecionado ? `${fornecedorSelecionado.nome} - ${fornecedorSelecionado.nif ?? ""} -` : fornecedorSearch}
                  onChange={(e) => { setFornecedorSearch(e.target.value); setShowFornecedorDD(true); setSelectedFornecedorId(undefined); }}
                  onFocus={() => setShowFornecedorDD(true)}
                  className="flex-1 px-2.5 text-xs focus:outline-none bg-transparent"
                />
                {selectedFornecedorId && (
                  <button onClick={() => { setSelectedFornecedorId(undefined); setFornecedorSearch(""); }} className="px-2 text-gray-400 hover:text-gray-600">×</button>
                )}
              </div>
              {showFornecedorDD && fornecedoresFiltrados.length > 0 && (
                <>
                  <div className="fixed inset-0 z-10" onClick={() => setShowFornecedorDD(false)} />
                  <div className="absolute z-20 mt-1 w-full rounded border border-gray-200 bg-white shadow-md max-h-40 overflow-y-auto">
                    {fornecedoresFiltrados.map((f) => (
                      <button
                        key={f.id}
                        onClick={() => { setSelectedFornecedorId(f.id); setFornecedorSearch(f.nome); setShowFornecedorDD(false); }}
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs font-semibold uppercase tracking-wide text-gray-500">Produto / Serviço</h2>
              </div>
              <button
                onClick={addLinha}
            ...
            <div className="overflow-x-auto rounded border border-gray-200">
              <table className="w-full min-w-[640px] text-xs">
                ...
                <tbody>
                  {itens.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-6 text-center text-gray-400">Adicione itens à fatura</td>
                    </tr>
                  ) : (
                    itens.map((item, i) => (
                      <LinhaItem
